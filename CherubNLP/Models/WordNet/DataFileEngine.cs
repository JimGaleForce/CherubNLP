...
namespace CherubNLP.Models
{
	...
	public class DataFileEngine : WordNetEngine
	{
		...
		public override string[] GetPartsOfSpeech() { return new List<string>(_dataFileDictionary.Keys).ToArray(); }
		...
		public override RelationType[] GetRelationTypes(string lemma, string partOfSpeech)
		{
			...
			if(indexWord != null)
			{
				if(indexWord.RelationTypes != null)
				{
					...
					{
						relationTypes[currentRelationType] = _relationTypeDictionary[
			...
			for(int currentRelation = 0; currentRelation < relationCount; currentRelation++)
			{
				...
				if(sourceTarget == 0)
				{
					relations[currentRelation] = new Relation(
						this,
						_relationTypeDictionary[relationTypeKey],
						targetSynsetOffset,
						targetPartOfSpeech);
				}
				else
		...
		private class PosDataFileSet : IThreadedResource<PosDataFileSet>
		{
			...
			public PosDataFileSet(string dataFolder, string partOfSpeech)
			{
				...
				IndexFile = new StreamReader(Path.Combine(dataFolder, "index." + partOfSpeech));
